...
export function Sidebar() {
  ...
  return (
    <div className={`${open ? "w-72" : "w-20"} hidden lg:block h-screen dark:bg-gray-400 border-r border-gray-300 relative duration-700 transition-all`}>
      <button
      ...
      <div className="flex flex-col items-center justify-center my-5">
        <img
          className={`duration-700 ${open ? "scale-100" : "scale-50"} w-32 border border-gray-300 rounded-full font-bold uppercase text-white shadow-2xl`}
        ...
        <div className="w-[100%] flex items-center justify-center my-10">
          <ul className="w-full flex flex-col gap-5 p-2">
            {menuLink.map((item, index) => (
              ...
              >
                ...
                <li className={`ease-in-out ${open ? "duration-700 scale-100" : "duration-300 scale-0"} text-2xl font-medium`}>{item.name}</li>
              </Link>
            ))}
          </ul>
          ...
          >
            ...
            <p className={`ease-in-out ${open ? "duration-700 scale-100" : "duration-300 scale-0"}  text-2xl font-medium`}>Sair</p>
          </Link>
        </div>
      </div>
    </div>
